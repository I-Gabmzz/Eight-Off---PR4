...
public class ControlDeMovimientos {
    ...
    private static final ListaDoble<ControlDeMovimientos> historial = new ListaDoble<>();

    // Puntero que indica donde estamos en el historial
    private static NodoDoble<ControlDeMovimientos> posicionActual = null;

    // Puntero para saber donde estaba el juego antes de abrir el visor
    ...
    // Logica para ejecutar un movimiento (hacia adelante)
    ...
    public static void restaurarPosicion(Tablero[] pilasTablero, Reserva[] celdasReserva, Foundation[] pilasFundacion) {
        while (posicionActual != posicionGuardada) {
            if (historial.estaVacia() || posicionGuardada == null) break; // Seguridad
    ...
    // Metodo para formatear el movimiento para el visor
    @Override
